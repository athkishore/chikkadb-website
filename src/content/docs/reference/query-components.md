...
The filter is a document that specifies the conditions to be matched during a command. It is an input component of several commands such as `count`, `find`, `findAndModify`, `update`, `delete`, etc.

This page lists the various formats in which filter expressions can be specified in a filter object, in accordance with the MongoDB query language. The following cases have been [tested](https://github.com/athkishore/chikkadb-ts/blob/main/tests/find/find.test.ts) for correctness.
...
#### 3. Explicit conditions using [filter operators](/reference/operators-and-expressions#filter-operators)
Examples:
```typescript
{
  "age": { "$gte": 25 }
...
#### 4. Composite conditions using [logical operators](/reference/operators-and-expressions#logical-operators)
Example:
```typescript
{
  $or: [
